...
class AuthSteps:
    ...
    def authorize_user(self, user: dict = None):
        if user is None:from data.user_data import UserRegistrationModel
from allure import step
from routes.routes import Routes
import requests
...
class AuthSteps:
    def __init__(self):
        self.login_endpoint = Routes().LOGIN_USER

    @step('Авторизация пользователя')
    def authorize_user(self, user: dict = None):
        if user is None:
            user = UserRegistrationModel().dict()
        response = requests.post(url=f'{Routes().BASE_URL}{self.login_endpoint}', json=user)
        return response
